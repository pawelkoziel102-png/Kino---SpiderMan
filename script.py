...
# Daty do sprawdzenia (premiery i weekend premierowy w 2026 roku)
DATES_TO_CHECK = ["2026-06-17", "2026-06-18", "2026-06-19"]
...
def send_email(date_found, formats_found):
    ...
    body = (
        ...
        f"Link do zakupu: https://www.cinema-city.pl/filmy/spider-man-calkiem-nowy-dzien/8085s2r"
    )
...
def check_tickets_for_date(date_str):
    url = f"https://cinema-city.pl{CINEMA_ID}/byDate/{date_str}"
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                print(f"Błąd: Serwer nie zwrócił JSON-a dla daty {date_str}.")
            ...
            if target_film:
                ...
                if imax_screenings:
                    return True, date_str, ["IMAX"]

                print(
                    f"[{date_str}] Film jest w bazie, ale brak seansów IMAX."
                )
                return False, None, None
            else:
                print(f"[{date_str}] Brak filmu w siatce na ten dzień.")
                return False, None, None
        else:
            print(
                f"Błąd API dla daty {date_str} (Status: {response.status_code})"
            )
            return False, None, None
    except Exception as e:
        print(f"Wystąpił błąd połączenia dla daty {date_str}: {e}")
        return False, None, None


if __name__ == "__main__":
    print("Sprawdzam bilety IMAX w CC Sadyba na weekend premierowy...")
    for date in DATES_TO_CHECK:
        tickets_out, date_found, formats_found = check_tickets_for_date(date)
        if tickets_out:
